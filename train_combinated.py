...
parser.add_argument('--dataset', help=f'Training dataset path (default: {WORK_PATH}/dataset/TrainCIC2019.csv)', default=WORK_PATH + '/dataset_clean/CICIDS2018.csv') # Required parameter
parser.add_argument('--model', help='Traning model selection (ANN, ANND(Droout), DNN, DNND(), CNN) (default: DNND)', default='DNND') 
...
parser.add_argument('--type', help='Type of dataset ("CICIDS2019","CICIDS2019_anomaly","CICIDS2019_test","CICIDS2018","CICIDS2018_anomaly","CICIDS2017","CICIDS2017_anomaly" default: CICIDS2018)', default='CICIDS2018') 
...
batch_size = args.bs
type = args.type
trained_model_dir_PATH = WORK_PATH + '/trained_model' + '/' + type + '/' + model_type + '_' + ver 
...
csv_plot_PATH = WORK_PATH + '/trained_model' + '/' + type + '/'  + model_type + '_' + ver + '.csv'
print('-------------PARAMETERS-------------')
...
CICIDS2019 = pd.read_csv('/home/CICIDS/dataset_ddos/CICIDS2019.csv')
CICIDS2018 = pd.read_csv('/home/CICIDS/dataset_ddos/CICIDS2018.csv')
CICIDS2017 = pd.read_csv('/home/CICIDS/dataset_ddos/CICIDS2017.csv')
#test = pd.read_csv('/home/jun/CICIDS2019/TestCIC2019.csv')
print("DONE!")
print("-------------------------")

dataset = {'CICIDS2017_anomaly':CICIDS2017,'CICIDS2018_anomaly':CICIDS2018,'CICIDS2019_anomaly':CICIDS2019}
X_train_combinated = []
y_train_combinated = []
X_test_combinated = []
y_test_combinated = []
X_val_combinated = []
y_val_combinated = []
for train_type,train in dataset.items():
    print(f'-------------Starting analyze {train_type}-------------')
    # Fix columns name
    ...
    print("-------------------------")
    X_train_combinated.append(X_train)
    y_train_combinated.append(y_train.reshape(y_train.shape[0],1))
    X_val_combinated.append(X_val)
    y_val_combinated.append(y_val.reshape(y_val.shape[0],1))
    X_test_combinated.append(X_test)
    y_test_combinated.append(y_test.reshape(y_test.shape[0],1))
    
X_train = np.concatenate(X_train_combinated)
y_train = np.concatenate(y_train_combinated)
y_train = y_train.reshape(y_train.shape[0],)
X_test = np.concatenate(X_test_combinated)
y_test = np.concatenate(y_test_combinated)
y_test = y_test.reshape(y_test.shape[0],)
X_val = np.concatenate(X_val_combinated)
y_val = np.concatenate(y_val_combinated)
y_val = y_val.reshape(y_val.shape[0],)
X_train_combinated.clear()
y_train_combinated.clear()
X_test_combinated.clear()
y_test_combinated.clear()
X_val_combinated.clear()
y_val_combinated.clear()
print(f"Train dataset: X[{X_train.shape}, y[{y_train.shape}]")
print(f"Validate dataset: X[{X_val.shape}, y[{y_val.shape}]")
...
f = open('./trained_model' + '/' + type + '/'  + model_type + '_' + ver + '.csv', 'w')
writer = csv.writer(f)
...
for epoch in range(epoch_start, num_epochs):
    f = open('./trained_model' + '/' + type + '/'  + model_type + '_' + ver + '.csv', 'a')
    writer = csv.writer(f)
    ...
    saved_model_PATH = './trained_model' + '/' + '/' + type + '/' + model_type + '_' + ver + '/' + str(epoch + 1) + 'epoch.pth'
